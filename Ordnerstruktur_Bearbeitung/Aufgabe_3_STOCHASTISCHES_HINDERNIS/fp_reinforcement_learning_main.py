...
for P in np.linspace(0.65,0.75,10):
	print(P)
	env = fp_classes.environment()
	env.P_obstacle = P
	learner = fp_classes.agent(env)
	...
	# ~ total_action_displacement = total_action_displacement/np.abs(total_action_displacement)	
	print(f"total_action_displacement/total_action_counter = {total_action_displacement/np.abs(total_action_displacement)}")
	THRESHOLD.append([P,total_action_displacement/np.abs(total_action_displacement)])
...
fig,ax = plt.subplots()
ax.plot(THRESHOLD[:,0],THRESHOLD[:,1])
ax.set_xlabel(r"P")
ax.set_ylabel(r"action direction")
fig.savefig("action_direction_" + now + ".png",bbox_inches="tight")
plt.show()
