...
for i in range(len(P_values)):
	...
	for episode in range(learner.N_episodes):
		...
		learner.x = env.starting_position #np.random.randint(0, env.N_states)
		...
		for t in range(learner.tmax_MSD):
			...
			if learner.epsilon == 0:
				total_action_displacement += (learner.choosen_action-1)
	...
	Kappa[i] = total_action_displacement/np.abs(total_action_displacement)
	#print('3.4.9: ', total_action_displacement/np.abs(total_action_displacement))
	print(i, (i+1)/len(P_values)*100, '%')


plt.figure();plt.title('number of P$_0$: %i' %len(P_values))
plt.plot(P_values, Kappa)
plt.ylabel('$\kappa$', fontsize=17);plt.xlabel('P$_0$', fontsize=17)
#plt.savefig('P_0_vs_Kappa_alpha=0_999999_'+now+'.png')
plt.show()
...
plt.ylabel('Anzahl der Schritte ',fontsize=17);plt.xlabel('Episode',fontsize=17)
#plt.savefig('learning curve with alpha = 0.01 and N_episodes = 1000 '+ now + '.png')

#plt.figure(figsize=(16,9));plt.title('learning curve count_steps/min_actions',fontsize=23)
#plt.semilogy(L_3_3_2_3[:,1], L_3_3_2_3[:,0]/min_actions)
#plt.ylabel('Anzahl der Schritte',fontsize=17);plt.xlabel('Episode',fontsize=17)
#plt.savefig('learning curve with random starting pos'+ now + '.png')
