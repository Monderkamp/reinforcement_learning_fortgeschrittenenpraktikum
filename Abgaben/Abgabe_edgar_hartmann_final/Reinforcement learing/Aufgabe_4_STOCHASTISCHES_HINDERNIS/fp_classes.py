...
class agent:
	...
	def stoch_obstacle(self,env_):
		if self.x >= env_.obstacle_interval[0]-1 and self.x <= env_.obstacle_interval[-1]:
			if np.random.rand() <= env_.P_obstacle:
				#print(env_.P_obstacle, self.x, env_.obstacle_interval[0], env_.obstacle_interval[-1])
				self.x -= 1
			else:
				self.x = self.x
