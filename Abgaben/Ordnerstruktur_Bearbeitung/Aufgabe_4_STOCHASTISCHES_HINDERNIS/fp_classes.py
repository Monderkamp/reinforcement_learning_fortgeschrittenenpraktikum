...
class environment:
	 def __init__(self):
		 ...
		 self.middle = self.N_states/2 + self.target_position #für x höher als dieser Wert wird der dog andersrum gehen lernen
...
class agent:
	def __init__(self,env_):
		...
		self.x_old = None
		self.output_state = 30 #has to be a value <= N_states
	...
	def stoch_obstacle(self,env_):
		if np.any(env_.obstacle_interval == self.x) and np.random.rand() <= env_.P_obstacle: #if dog in intervals and for probability of P_obstacle
			self.x -= 1
