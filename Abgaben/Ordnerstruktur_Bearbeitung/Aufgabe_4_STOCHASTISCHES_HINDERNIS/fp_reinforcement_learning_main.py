...
for n in tqdm(range(11)):
	...
	for episode in range(learner.N_episodes):
		...
		# learner.x = np.random.randint(0,env.N_states) #random position
...
ax.scatter(P,KAPPA)
ax.set_xlabel('P_obstacle')
ax.set_ylabel('$\kappa$')
ax.set_title('Choice of direction')
