...
class agent:
	...
	def choose_action(self):
		...
		if self.rnd <= self.epsilon:
			...
			self.chosen_action = np.argwhere(self.Q[self.x,:] == np.max(self.Q[self.x,:])) #gibt indices wo self.Q das/die Maximum/ma hat
	...
	def perform_action(self,env_):
		...
		if self.x >= self.xmax:
			self.x = 0 #denke wenn x den zst in letzter zeile erreicht hat fängt er wieder bei zst 0 o. 1 an ?
		elif self.x < 0:                #oder 1?
			self.x = self.xmax - 1
		return self.x
	...
	def update_Q(self,env_):
		...
		self.bracket = self.target_reward*(self.x==env_.target_position) + self.gamma*np.max(self.Q[self.x,:]) - self.Q[self.x_old,self.chosen_action]
		self.Q[self.x_old,self.chosen_action] += self.alpha * self.bracket
